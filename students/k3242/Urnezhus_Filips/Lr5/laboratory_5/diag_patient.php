...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if (isset($_POST["delete"])) {
        $sql = 'DELETE from lab3."Diag.Patient" where "id_patient" = :id_patient';
    ...
    if (isset($_POST["find"])) {
        $sql = 'SELECT * from lab3."Diag.Patient" where "id_patient" = :id_patient';
        $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
        $sth->execute(array(':id_patient' => $_POST["id_patient"]));
        $data = $sth->fetchAll();
    ...
    if (isset($_POST["edit"])) {
        ...
        if($_POST["id_patient"] != "" && count($data) > 0){
            $sql = 'UPDATE lab3."Diag.Patient" SET "id_diagnosis" = :id_diagnosis, "date_diagnosis" = :date_diagnosis where "id_patient" = :id_patient';
            $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
            $sth->execute(array(':id_patient' => $_POST["id_patient"],':date_diagnosis' => $_POST["date_diagnosis"],':id_diagnosis' => $_POST["id_diagnosis"]));
            $data = $sth->fetchAll();
            print_r ($sth->errorInfo()[2]);
    ...
    if (isset($_POST["add"])){
        $sql = 'INSERT INTO lab3."Diag.Patient"("id_diagnosis", "date_diagnosis") VALUES (:id_diagnosis, :date_diagnosis)';
        $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
        $sth->execute(array(':id_diagnosis' => $_POST["id_diagnosis"],':date_diagnosis' => $_POST["date_diagnosis"]));
        $data = $sth->fetchAll();
        print_r ($sth->errorInfo()[2]);
...
<form action="" method="post">
    <input name="id_patient" placeholder="id пациента..." value="<?php echo '' ?>"></br>
...
<form action="" method="post">
    ...
    <input name="date_diagnosis" size="40" placeholder="Дата диагноза..." value="<?php if($_SERVER['REQUEST_METHOD'] == 'POST' && $data) echo $data[0]['date_diagnosis']?>"> <-Дата диагноза</br>
