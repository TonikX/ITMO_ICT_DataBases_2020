...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if (isset($_POST["delete"])) {
        $sql = 'DELETE from lab3."Med.Card" where "id_patient" = :id_patient';
    ...
    if (isset($_POST["find"])) {
        $sql = 'SELECT * from lab3."Med.Card" where "id_patient" = :id_patient';
        $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
        $sth->execute(array(':id_patient' => $_POST["id_patient"]));
        $data = $sth->fetchAll();
    ...
    if (isset($_POST["edit"])) {
        ...
        if($_POST["id_patient"] != "" && count($data) > 0){
            $sql = 'UPDATE lab3."Med.Card" SET "contacts" = :contacts, "birthday" = :birthday, "full_name" = :full_name where "id_patient" = :id_patient';
            $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
    ...
    if (isset($_POST["add"])){
        $sql = 'INSERT INTO lab3."Med.Card"("contacts", "birthday", "full_name") VALUES (:contacts, :birthday, :full_name)';
        $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
