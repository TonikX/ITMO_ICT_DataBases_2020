...
<!-- Latest compiled and minified JavaScript -->
    ...
    <nav class="navbar navbar-default">
        <div class="">
            <div>
                <a class="navbar-brand" style="margin-left: 46%" href="breed.php">Breed</a>
            </div>
        </div>
    </nav>
...
<?php
    ...
    try {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if (isset($_POST["find"])) {
                $sql = 'SELECT * from show."Breed" where "Arena_number" = :arena_num';
            ...
            if (isset($_POST["edit"])) {
                ...
                if ($exist) {
                    $sql = 'UPDATE show."Breed" SET "Arena_number"= :arena_num, "Breed_name"= :breed_name
                    where "Arena_number" = :arena_num';
                    $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                    $sth->execute(array(':arena_num' => $_POST["arena_num"], ':breed_name' => $_POST["breed_name"]));
                    ...
                    $sql = 'INSERT INTO show."Breed"("Arena_number", "Breed_name") VALUES (:arena_num, :breed_name)';
                    $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                    $sth->execute(array(':arena_num' => $_POST["arena_num"], ':breed_name' => $_POST["breed_name"]));
        ...
        if($_SERVER['REQUEST_METHOD'] == 'GET'){
            if($_GET["a_num"] != "") {
                $sql = 'delete from show."Breed" where "Arena_number" = :arena_num';
                $sth = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
...
<div class="layer1" align="left">
    <form action="breed.php" method="post">
        ...
        <input name="breed_name" size="30" placeholder="breed name..." value="<?php if($_SERVER['REQUEST_METHOD'] == 'POST' && $value) echo $value[0]['Breed_name']?>"> - Порода</br>
